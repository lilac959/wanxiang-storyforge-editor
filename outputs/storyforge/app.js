...
function finish(){setPlayerPhase('complete');clearInterval(timer);$('#playerStatus').textContent='作品试玩完成';$('#playerScreen').innerHTML=`<div class="stage settingsstage">${scenery()}<div class="loadcontent introtitle"><p>THE END</p><h2>山海有归，故事未尽</h2><p>${esc(project.name)}</p><button id="again">再走一条不同的路 →</button></div></div>`;$('#again').onclick=()=>start(true)}
...
async function init(){try{const p=JSON.parse(localStorage.getItem('storyforge-project'));repairTiming(p);if(valid(p))project=p}catch{}if(project.name==='雾隐 · 山海之间'){project.name='万象环轨';save()}if(configureChapterOne(project))save();if(installNineVideos(project))save();if(applyQteTimingDefaults(project))save();if(installDeathNode(project))save();if(installLinkedTailTiming(project))save();if(installMouseQte(project))save();if(installTailUx(project))save();if(installCinema(project))save();if(migrateCinemaTimeline(project))save();if(installCinemaWidthB(project))save();if(installBranchGrayscale(project))save();selected=project.nodes[0].id;page='loading';render();if(!project.loading.coverRevision){Object.assign(project.loading,{image:COVER_ID,title:'',subtitle:'',text:'正在加载资源',color:'#e5d6b1',coverRevision:1});save()}if(!project.loading.titleRevision){Object.assign(project.loading,{title:'万象环轨',titleLayout:'square',titleRevision:1});save()}try{const cover=await getBlob(COVER_ID);if(!cover){const response=await fetch('assets/loading-cover.png');if(response.ok)await putAsset(COVER_ID,new Blob([await response.blob()],{type:'image/png'}))}else if(cover.type!=='image/png')await putAsset(COVER_ID,new Blob([cover],{type:'image/png'}))}catch{}if(!project.splash.openingRevision){Object.assign(project.splash,{title:'',subtitle:'',video:'asset-opening-v1',videoName:'cgt-20260911152112-z8h9n.mp4',openingRevision:1,waitForStart:true,skip:false});save()}try{if(!await getBlob('asset-opening-v1')){const response=await fetch('assets/opening.mp4');if(response.ok)await putAsset('asset-opening-v1',new Blob([await response.blob()],{type:'video/mp4'}))}}catch{}if(!project.loading.videoRevision){Object.assign(project.loading,{video:'asset-loading-video-v1',videoName:'cgt-20260911164747-ct8t8.mp4',videoRevision:1});save()}try{if(!await getBlob('asset-loading-video-v1')){const response=await fetch('assets/loading-background.mp4');if(response.ok)await putAsset('asset-loading-video-v1',new Blob([await response.blob()],{type:'video/mp4'}))}}catch{}for(let i=1;i<=4;i++){const id='asset-chapter01-0'+i;try{if(!await getBlob(id)){const response=await fetch('assets/chapter01-0'+i+'.png');if(response.ok)await putAsset(id,new Blob([await response.blob()],{type:'image/png'}))}}catch{}}for(const id of new Set(projectAssets(project).filter(x=>x?.startsWith('asset-')))){const b=await getBlob(id);if(b)media.set(id,URL.createObjectURL(b))}render()}init();
