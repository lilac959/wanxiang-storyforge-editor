...
function stop(){clearTailQte();clearGenericMouse();clearSplashInput();loadingAbort?.abort();loadingAbort=null;clearPreparedVideos();clearInterval(boardTimer);runToken++;activeRun=false;clearInterval(timer);$('#playerScreen').querySelectorAll('video').forEach(v=>v.pause());liveVideo=null;liveActions=null}
...
function clearPreparedVideos(){
 for(const video of preparedVideos.values()){video.pause();video.removeAttribute('src');video.load()}
 preparedVideos.clear();
}
function prepareVideoResource(id,url,signal){return new Promise((resolve,reject)=>{
 const previous=preparedVideos.get(id);
 if(previous){previous.pause();previous.removeAttribute('src');previous.load();preparedVideos.delete(id)}
 const video=document.createElement('video');let settled=false;
 const abort=()=>finish(new DOMException('已取消加载','AbortError'));
 const finish=error=>{if(settled)return;settled=true;clearTimeout(timeout);signal.removeEventListener('abort',abort);video.onloadeddata=null;video.onerror=null;if(error){video.removeAttribute('src');video.load();reject(error)}else{preparedVideos.set(id,video);resolve()}};
 const timeout=setTimeout(()=>finish(Error('素材预解码超时，请检查文件格式')),30000);
 if(signal.aborted){abort();return}signal.addEventListener('abort',abort,{once:true});
 video.preload='auto';video.muted=true;video.playsInline=true;video.onloadeddata=()=>finish();video.onerror=()=>finish(Error('素材无法解码，请替换视频'));video.src=url;video.load();
})}
function takePreparedVideo(id,placeholder){
 const video=preparedVideos.get(id);if(!video||!placeholder)return placeholder;
 preparedVideos.delete(id);video.pause();video.onended=null;video.onerror=null;video.onloadedmetadata=null;video.ontimeupdate=null;video.className=placeholder.className;video.playsInline=true;video.loop=placeholder.loop;video.controls=placeholder.controls;video.muted=false;video.playbackRate=1;placeholder.replaceWith(video);return video;
}
...
async function prepareResource(task,signal,update){
 ...
 try{
  if(!url?.startsWith('blob:')&&task.id?.startsWith('asset-')){const stored=await getBlob(task.id);if(stored){url=URL.createObjectURL(stored);media.set(task.id,url)}}
  ...
  if(!url.startsWith('blob:')&&!url.startsWith('data:')){
   ...
   try{task.kind==='video'?await prepareVideoResource(task.id,loadedUrl,signal):await decodeResource(loadedUrl,task.kind,signal)}catch(error){URL.revokeObjectURL(loadedUrl);throw error}
  ...
  }else task.kind==='video'?await prepareVideoResource(task.id,url,signal):await decodeResource(url,task.kind,signal);
...
function playSplash(token){
  ...
  let advanced=false;const v=takePreparedVideo(c.video,$('#playerScreen video')),screen=$('#playerScreen .splash-stage');
