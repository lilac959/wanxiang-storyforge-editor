...
(async()=>{
 ...
 try{
  ...
  const media='/api/media/'+id.slice(12);r=await fetch(game+media,{headers:{Range:'bytes=0-7'}});assert.equal(r.status,206);assert.equal((await r.arrayBuffer()).byteLength,8);
  ...
  assert.equal((await (await fetch(game+'/api/project')).json()).version,first.version);
  const player=await browser.newPage();await player.addInitScript(()=>{const original=window.setInterval;window.setInterval=(fn,ms,...args)=>original(fn,ms===30000?200:ms,...args)});player.on('pageerror',e=>errors.push(e.message));await player.goto(game);await player.waitForFunction(()=>document.querySelector('#gameStart').disabled===false);assert.equal(await player.locator('#gameTitle').textContent(),'同步测试一');assert.equal(await player.locator('.app').isVisible(),false);
  ...
  await player.locator('#gameStart').click();await player.waitForFunction(()=>project.name==='同步测试二');assert(await player.locator('#player').isVisible());
  await player.locator('.start-prompt').waitFor({state:'visible',timeout:15000});await player.keyboard.press('Space');await player.waitForSelector('#playerScreen .choices button',{state:'visible',timeout:15000}).catch(async e=>{console.log(await player.locator('#playerScreen').innerText());throw e});assert.equal(await player.locator('#playerScreen .choices button').textContent(),'结束');await p.evaluate(()=>localStorage.setItem('storyforge-publish-token','invalid-test-key'));await p.locator('#projectName').fill('同步测试三');await p.locator('#saveConfig').click();await p.waitForFunction(()=>!cloudSaving);assert.match(await p.locator('#saved').textContent(),/同步失败/);assert.equal((await (await fetch(game+'/api/project')).json()).project.name,'同步测试二');assert.equal(await p.evaluate(()=>JSON.parse(localStorage.getItem('storyforge-project')).name),'同步测试三');
  await p.evaluate(token=>localStorage.setItem('storyforge-publish-token',token),token);await p.locator('#saveConfig').click();await p.waitForFunction(()=>!cloudSaving);await player.locator('#gameUpdate').waitFor({state:'visible'});assert.equal(await player.evaluate(()=>project.name),'同步测试二');await player.locator('#gameUpdate').click();await player.waitForFunction(()=>project.name==='同步测试三');assert.deepEqual(errors,[]);
  console.log('PASS save+asset upload, independent player, latest start, byte ranges, write authorization, atomic failure, stale version conflict, real playback');
